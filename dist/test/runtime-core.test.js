...
import { buildCommercialReceipt, buildCommonsReceipt, buildSchemaPath, canonicalizeReceipt, COMMAND_LAYER_CURRENT_LINE, COMMONS_CONTRACT, COMMERCIAL_CONTRACT, createLayeredReceipt, extractEd25519Raw32FromSpkiDer, hashReceiptCanonical, normalizeCommercialRequest, normalizeRequest, parsePemToDer, signReceiptEd25519Sha256, signReceiptEd25519, toBase64Url, toLegacyReceiptEnvelope, toLegacySignedReceipt, verifyReceiptEd25519Sha256, verifyReceiptSignature } from '../index.js';
...
test('buildSchemaPath uses the v1.1.0 current-line layout', () => {
    assert.equal(buildSchemaPath({ contract: COMMONS_CONTRACT, verb: 'chat.completions', kind: 'request' }), '/schemas/1.1.0/commons/chat.completions/v1/request.schema.json');
    assert.equal(buildSchemaPath({ contract: COMMERCIAL_CONTRACT, verb: 'chat.completions', version: 'v2', kind: 'receipt' }), '/schemas/1.1.0/commercial/chat.completions/v2/receipt.schema.json');
});
...
test('createLayeredReceipt preserves current-line receipt/runtime separation', () => {
    ...
    const commercial = createLayeredReceipt(buildCommercialReceipt({
        verb: 'test.verb',
        version: 'v1',
        commercial: { plan: 'pro' },
...
test('sign/verify pipeline keeps proof outside the canonical receipt', () => {
    ...
    const receipt = signReceiptEd25519(buildCommercialReceipt({
        verb: 'test.verb',
        version: 'v1',
