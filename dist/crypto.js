...
export const ENS_KEY_PUB = "cl.sig.pub";
export const ENS_KEY_KID = "cl.sig.kid";
export const ENS_KEY_CANONICAL = "cl.sig.canonical";
export const ENS_KEY_SIGNER = "cl.receipt.signer";
 ...
 */
export function encodePublicKey(rawBytes) {
    if (rawBytes.length !== 32) {
        throw new Error(`Ed25519 public key must be 32 bytes, got ${rawBytes.length}`);
    }
 ...
 */
export function parsePublicKey(ensPubValue) {
    const prefix = "ed25519:";
    if (!ensPubValue.startsWith(prefix)) {
        throw new Error(`cl.sig.pub must start with "ed25519:", got: ${ensPubValue.slice(0, 20)}`);
    }
    const b64 = ensPubValue.slice(prefix.length);
    const raw = Buffer.from(b64, "base64");
    if (raw.length !== 32) {
        throw new Error(`cl.sig.pub decoded to ${raw.length} bytes; expected 32 (Ed25519 public key)`);
    }
 ...
 */
export function signCanonical(canonicalString, privateKeyPem) {
    const sign = createSign("Ed25519");
    sign.update(canonicalString, "utf8");
    sign.end();
    const sigBuffer = sign.sign(privateKeyPem);
    if (sigBuffer.length !== 64) {
        throw new Error(`Ed25519 signature must be 64 bytes, got ${sigBuffer.length}`);
    }
 ...
 */
export function verifyCanonical(canonicalString, signatureBase64, publicKeyPem) {
    const sigBuffer = Buffer.from(signatureBase64, "base64");
    if (sigBuffer.length !== 64) {
        throw new Error(`Signature must decode to 64 bytes (Ed25519), got ${sigBuffer.length}`);
    }
    try {
 ...
 */
export function verifyCanonicalWithRawKey(canonicalString, signatureBase64, rawPublicKey) {
    if (rawPublicKey.length !== 32) {
        throw new Error(`Raw public key must be 32 bytes, got ${rawPublicKey.length}`);
    }
...
export function generateEd25519KeyPair() {
    ...
    const spkiDer = Buffer.from(publicKey
        .replace(/-----[^-]+-----/g, "")
        .replace(/\s/g, ""), "base64");
